...
static inline bool accepts_lines(cmark_node_type block_type) {
  return (block_type == CMARK_NODE_PARAGRAPH || block_type == CMARK_NODE_HEADER ||
...
static int break_out_of_lists(cmark_parser *parser, cmark_node **bptr) {
  ...
  // find first containing CMARK_NODE_LIST:
...
static void S_process_line(cmark_parser *parser, const unsigned char *buffer,
  ...
  while (container->last_child && container->last_child->open) {
    ...
    } else if (container->type == CMARK_NODE_RAW_BLOCK) {
  ...
  while (container->type != CMARK_NODE_CODE_BLOCK && container->type != CMARK_NODE_RAW_BLOCK) {
    ...
    if (!indented && peek_at(&input, parser->first_nonspace) == '>') {
      ...
      if (peek_at(&input, parser->offset) == ' ')
        S_advance_offset(parser, &input, 1, false);
      container =
          add_child(parser, container, CMARK_NODE_BLOCK_QUOTE, parser->offset + 1);
      ...
      S_advance_offset(parser, &input,
                       ...
                       false);
      container = add_child(parser, container, CMARK_NODE_HEADER, parser->offset + 1);
      ...
      container =
          add_child(parser, container, CMARK_NODE_RAW_BLOCK, parser->first_nonspace + 1);
      container->format = CMARK_FORMAT_HTML;
      ...
      container =
          add_child(parser, container, CMARK_NODE_HRULE, parser->first_nonspace + 1);
      S_advance_offset(parser, &input, input.len - 1 - parser->offset, false);
      ...
      // add the list item
      container =
          add_child(parser, container, CMARK_NODE_ITEM, parser->first_nonspace + 1);
      /* TODO: static */
      memcpy(&container->as.list, data, sizeof(*data));
      free(data);
      ...
      S_advance_offset(parser, &input, CODE_INDENT, true);
      container =
          add_child(parser, container, CMARK_NODE_CODE_BLOCK, parser->offset + 1);
  ...
  container->last_line_blank =
      (parser->blank && container->type != CMARK_NODE_BLOCK_QUOTE &&
       ...
       !(container->type == CMARK_NODE_CODE_BLOCK && container->as.code.fenced) &&
